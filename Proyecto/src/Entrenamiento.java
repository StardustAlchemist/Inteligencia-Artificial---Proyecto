...
public class Entrenamiento {
	...
	ArrayList<String> list = new ArrayList<String>(); // Listado de las lineas del archivo de texto (palabras +
														// etiqueta)
	...
	ArrayList<BigDecimal> probabilidadesEtiquetas = new ArrayList<BigDecimal>(); // Probabilidades individuales de las
																					// etiquetas
	ArrayList<Integer> frecuenciaFraseEtiqueta = new ArrayList<Integer>(); // Lista para guardar las frecuencias de cada
																			// etiqueda en las frases
	ArrayList<TablaFrecuencias> tabla = new ArrayList<TablaFrecuencias>(); // tabla de frecuencias
	...
	public void LeerTexto(String NombreArchivo) // Mï¿½todo para leer el archivo de entrenamiento.
	{
		...
		try {
			...
			// FileInputStream(NombreArchivo), "ISO-8859-1"));
	...
	private void DividirPalabras(ArrayList<String> listadoLineas) {
		...
		for (int i = 0; i < listadoLineas.size(); i++) {
			...
			palabras.add(StringUtils.lowerCase(StringUtils.stripAccents(palabraEtiqueta[0]).trim())); // Se almacena la
																										// linea de
																										// palabras de
																										// la linea
																										// completa.
		...
		for (int i = 0; i < lineaDePalabras.size(); i++) {
			...
			for (int j = 0; j < lineaSeparar.length; j++) {
				if (lineaSeparar[j].length() != 0 && !lineaSeparar[j].matches("\\W")) {
					...
					if (!IgnoreCase(listadoPalabras, lineaSeparar[j])) // Se evalua si la palabra se encuentra o no en
																		// la lista.
					{
	...
	private void CrearTablaFrecuencias() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (int j = 0; j < separarOracion.length; j++) // Se recorre el arreglo de las palabras de la frase que se
															// esta revisando
			{
				// -------------------------
				if (separarOracion[j].length() != 0 && !separarOracion[j].matches("\\W")) {
					...
					if (!IgnoreCase(palabrasRevisadas, separarOracion[j])) {
						...
						tabla.add(new TablaFrecuencias(separarOracion[j].trim(), etiqueta.trim(), i + 1,
								frecuenciaPalabra));
						frecuenciaPalabra = 0;
					}
				}
	...
	private void FrecuenciaEtiqueta(ArrayList<TablaFrecuencias> TablaFrecuencia, ArrayList<String> LEtiquetas) {
		...
		for (int i = 0; i < LEtiquetas.size(); i++) {
			...
			for (int j = 0; j < TablaFrecuencia.size(); j++) {
				...
				if (!NoFrases.contains(TablaFrecuencia.get(j).nofrase())
						&& etiqueta.equalsIgnoreCase(TablaFrecuencia.get(j).etiqueta())) {
					NoFrasesEtiqueta++;
	...
	private void splitLoad(ArrayList<TablaFrecuencias> TablaFrecuencia) {
		if (listadoPalabras.size() < 500) {
			CrearBagOfWords(TablaFrecuencia, 0, listadoPalabras.size());
		} else {
			int dividir = Math.round(listadoPalabras.size() / 4);
			int inicio1 = 0;
			int fin1 = dividir;
			int inicio2 = fin1 + 1;
			int fin2 = dividir + inicio2;
			int inicio3 = fin2 + 1;
			int fin3 = dividir + inicio3;
			int inicio4 = fin3 + 1;
			int fin4 = listadoPalabras.size();

			Thread thread = new Thread() {
				public void run() {
					CrearBagOfWords(TablaFrecuencia, inicio1, fin1);
				}
			};
			thread.start();
			Thread thread2 = new Thread() {
				public void run() {
					CrearBagOfWords(TablaFrecuencia, inicio2, fin2);
				}
			};
			thread2.start();
			Thread thread3 = new Thread() {
				public void run() {
					CrearBagOfWords(TablaFrecuencia, inicio3, fin3);
				}
			};
			thread3.start();
			Thread thread4 = new Thread() {
				public void run() {
					CrearBagOfWords(TablaFrecuencia, inicio4, fin4);
				}
			};
			thread4.start();
		}
	...
	private void CrearBagOfWords(ArrayList<TablaFrecuencias> TablaFrecuencia, int inicio, int fin) {
		...
		for (int i = inicio; i < fin; i++) {
			...
			for (int j = 0; j < listadoEtiquetas.size(); j++) {
				...
				for (int k = 0; k < TablaFrecuencia.size(); k++) {
					if (palabra.equalsIgnoreCase(TablaFrecuencia.get(k).palabra())
							&& etiqueta.equalsIgnoreCase(TablaFrecuencia.get(k).etiqueta())) {
